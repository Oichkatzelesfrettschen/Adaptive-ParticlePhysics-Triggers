# This is without time dependence
import numpy as np
import random
import os
...
mc_bkg_jets, mc_bkg_ht = process_h5_file0("../data_Run_2016_283876.h5") #Data/data_Run_2016_283876.h5
# mc_bkg_jets, mc_bkg_ht = process_h5_file_mc("../MinBias_1.h5") #Data/MinBias_1.h5
#X1 = mc_bkg_jets[::100]
...
# np.random.shuffle(indices) # remove shuffle by Zixin to preserve time dependency
...
mc_AA_jets, mc_AA_ht = process_h5_file_mc("../HToAATo4B.h5") #Data/HToAATo4B.h5
X_AA = mc_AA_jets
...
mc_tt_jets, mc_tt_ht = process_h5_file_mc("../TT_1.h5") #Data/TT_1.h5
X_tt = mc_tt_jets
...
X_train, X_test, Jets1_train, Jets1_test, Npv1_train, Npv1_test, HT_train, HT_test = train_test_split(X1, Jets1, Npv1, HT1, test_size=0.8, random_state=42) #originally 0.5 by Shaghayegh and move to 0.8 by Zixin
...
for k in range(K):
    ...
    X_train, X_test = train_test_split(X_sub, test_size=0.8, random_state=40) # 0.5 - > 0.8 for Zixin #time dependence try

    for j, dim in enumerate(Dim):
        
        encoder, decoder = build_autoencoder0(IMG_SHAPE_X, dim) #try build autoencoder0
...
plt.title("10 Subsamples of size 50k, with 20% test size", fontsize=14) 
plt.grid(alpha=0.3)
plt.xticks(Dim)
plt.legend()
plt.tight_layout()
out_path = Path("RealData/paper_data/time_dependent_v4_seeds_build_autoencoder0.pdf")
